...
void test_cc(int num_arguments, char** argument_array) {
  if (num_arguments != 2) {
    fprintf(stderr, "Usage: %s filename.mtx\n", argument_array[0]);
    exit(1);
  }
  // --
  // Define types
  ...
  thrust::device_vector<vertex_t> component_ids(n_vertices);
  ...
  for (auto i = 0; i < num_runs; i++)
    gpu_elapsed += gunrock::cc::run(G, component_ids.data().get());
